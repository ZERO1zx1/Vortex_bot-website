...
class RRSelectModal(ui.Modal, title="Reaction role нэмэх"):
    emoji = ui.TextInput(label="Emoji", placeholder="😀", max_length=10)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        emoji_raw = str(self.emoji.value).strip()
        if not emoji_raw:
...
class RRSetupView(ui.View):
    ...
    @discord.ui.button(label="🗑️ Устгах", style=discord.ButtonStyle.danger)
...
class ReactionRoles(SupabaseCog):
    ...
    @app_commands.describe(action="Үйлдэл")
    ...
    async def rr_setup(self, interaction: discord.Interaction, action: app_commands.Choice[str]):
        ...
        if action.value == "setup":
            await self.db.execute("sql", {
                "query": f"CREATE TABLE IF NOT EXISTS {TABLE} (guild_id TEXT, message_id TEXT, emoji TEXT, role_id TEXT, created_at TEXT, PRIMARY KEY (guild_id, message_id, emoji))"})
